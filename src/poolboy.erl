...
handle_checkin(Pid, #state{overflow = Overflow, overflow_ttl = OverflowTtl} = State, kill) when Overflow > 0, OverflowTtl > 0 ->
    reap_worker(Pid, State);
handle_checkin(Pid, #state{overflow = Overflow} = State, kill) when Overflow > 0 ->
